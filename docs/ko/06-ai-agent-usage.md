...
Requirement:
  ...
  requirement가 loop 이후에도 유지될 수 있다면 rationale, failure prevented, assumptions, revisit 조건을 포함
  더 큰 root goal의 일부라면 현재 좁은 stage를 명시
...
Automation boundary:
  state-changing work라면 지금 자동화해도 되는 일, human review가 필요한 일, 아직 자동화하지 말아야 할 일을 명시
```
