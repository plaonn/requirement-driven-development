...
RDD는 먼저 일반적인 software work에 적용됩니다: pull request, ticket, task, 작은 iteration. Agent는 implementation momentum을 따라 adjacent work로 확장되기 쉽기 때문에 같은 구조에서 특히 이점을 얻습니다.
...
Spec:
  Rules, contracts, acceptance criteria, boundaries
...
- Refactoring task가 behavior change를 숨기면 안 됩니다.
- Bug fix가 명시적 re-scope 없이 related workflow redesign이 되면 안 됩니다.

## Final trace

작업 끝에 agent는 다음 trace를 요약해야 합니다.
